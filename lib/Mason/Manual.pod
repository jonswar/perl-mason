...
In either case, Mason searches under the component root for the provided path
with either a ".pm" or a ".m" attached, and then for a component called
"dhandler.pm" or "dhandler.m" ("directory handler").

If it cannot find any of these, then Mason scans backwards through the path,
checking each directory upwards for the same set of files. In this case it sets
$m->path_info to the unmatched latter part of the path.
...
noted.
    ...
    /news/sports.pm
    /news/sports.m
    /news/sports/dhandler.pm
    /news/sports/dhandler.m
    /news.pm                  # $m->path_info = sports
    /news.m                   # $m->path_info = sports
    /news/dhandler.pm         # $m->path_info = sports
    /news/dhandler.m          # $m->path_info = sports
    /dhandler.pm              # $m->path_info = news/sports
    /dhandler.m               # $m->path_info = news/sports
...
The default C<dispatch> calls C<render> (see below) if path_info is empty, and
otherwise declines the request. That is, by default, components will only
handle their exact path with nothing extra on it. If you want to accept and
parse the path_info, you'll have to implement a C<dispatch>.

=head2 The render method and content wrapping

The C<render> method is designed to be called in an inverted manner, from
superclasses down to subclasses. This is useful for creating general templates
that wrap around more specific pages.

By default, every component gets a C<render> method which will call C<inner()>
until the innermost component is reached, at which point it calls C<main()>
which displays the main component body.
...
Requesting /product/sales/display will ultimately cause Mason to call render on
/product/sales/display.m. Its content will be wrapped by two templates: the
outermost in /Base.pm, and the next one in /product/Base.pm.

Note that neither /product/sales/Base.pm nor /product/sales/display.m need to
define a C<render> method. A default render will be defined automatically and
will do the right thing.

If you want a component to ignore its superclass's wrappers, just override
C<render> to call C<main>:

    <%class>
    <%method render><% $self->main %></%method>
