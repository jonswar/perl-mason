...
components.  An article page for a news publication, for example, might call
...
Whenever possible, we recommend using regular Perl classes and methods outside
of Mason rather than returning values from components. The former encourages a
cleaner design.
