...
Mason 2.x is a vastly different beast than its predecessors, coming ten and
twelve years after Mason 1.0 and the original Mason respectively. It has been
rearchitected and reimplemented from the ground up. That said, the philosophy
and core syntax remain similar enough that it will be recognizable to existing
users.
...
call parameters will be passed to the constructor and placed in attributes.
...
been eliminated out of a perceived lack of utility.

=item *

C<escape_flags> was eliminated; define filters instead.
...
C<preloads> was eliminated; this code does roughly the same:
...
C<exec> was renamed to L<run|Mason::Interp/run>.
...
C<cache_self> was eliminated; use the L<Cache filter|Mason::Plugin::Cache>
instead.

=item *

C<callers>, C<caller> and C<caller_args> have been eliminated; now too
difficult to implement efficiently.

=item *

C<call_next> was replaced with Moose's C<inner>.

=item *

C<call_self> was eliminated; use L<filters|Mason::Manual::Filters> instead.

=item *

C<current_comp> was eliminated. Within a component, use C<< $self >>; outside a
component you can call L<Mason::Request/current_comp_class>, which will at
least get you the class.

=item *

C<dhandler_arg> was renamed to L<path_info|Mason::Request/path_info>.

=item *

C<exec> was renamed to L<run|Mason::Request/run>.

=item *

C<fetch_comp> was renamed to L<load|Mason::Request/load>.

=item *

C<subexec> was replaced with L<visit|Mason::Request/visit> and
