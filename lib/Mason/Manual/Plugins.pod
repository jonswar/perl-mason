
=head1 NAME

Mason::Manual::Plugins - Mason plugins

=head1 DESCRIPTION

A Mason plugin modifies behavior in one or more of Mason's main classes
simultaneously, using Moose roles. Many Mason features, even some that might be
considered "core", are implemented with plugins.

=head1 FINDING PLUGINS

By convention plugins live in the "Mason::Plugin::*" namespace, and plugin
bundles live in the "Mason::PluginBundle::*" namespace. You can find both with
this search:

    http://search.cpan.org/search?query=Mason%3A%3APlugin&mode=all

=head1 USING PLUGINS

Pass a plugins list to the Mason constructor:

    Mason->new(plugins =>
            [
             'OnePlugin', 
             'AnotherPlugin',
             '+My::Mason::Plugin::AThirdPlugin',
             '@APluginBundle',
             '+My::Mason::PluginBundle::AnotherBundle',
            ]);

Each entry can be either

=over

=item *

A plugin name, which will have "Mason::Plugin::" prepended to it.

=item *

A bundle name, prefixed with '@', which will have "Mason::PluginBundle::"
prepended to it.

=item *

A full plugin or bundle classname prefixed with '+'.

=back

=head1 CREATING PLUGINS

A plugin consists of the main plugin class and one or more roles. The main
class currently looks like this:

    package Mason::Plugin::MyPlugin;
    use Moose;
    with 'Mason::Plugin';

    1;

    __END__

    =pod

    =head1 NAME

    Mason::Plugin::MyPlugin - My plugin

    ....

That is, it doesn't have to do anything other than include the role
'Mason::Plugin', and document itself. The real action is in the roles, which
live underneath, and each modify a single Mason class:

    package Mason::Plugin::MyPlugin::Interp;
    use Moose::Role;

    # Modify Mason::Interp

    ...

    package Mason::Plugin::MyPlugin::Compiler;
    use Moose::Role;

    # Modify Mason::Compiler

    ...

When a plugin is applied, each of its roles will be automatically applied to
the appropriate Mason class.

Private or app-specific plugins need not be named "Mason::Plugin::*"; call them
whatever you want and use the '+' prefix to refer to them.

=head1 CREATING PLUGIN BUNDLES

A plugin bundle collects one or more plugins and/or bundles. It looks like
this:

    package Mason::PluginBundle::MyBundle
    use Moose;
    with 'Mason::PluginBundle';
    
    sub provides_plugins {
        return (
            'A',
            'B',
            '+My::Plugin::C',
            '@D',
            '+My::PluginBundle::E',
            );
    }

    1;

    __END__

    =pod

    =head1 NAME

    Mason::PluginBundle::MyBundle - My plugin bundle

    ....

The C<provides_plugins> method returns a list of entries, with the same syntax
as the C<plugins> parameter to C<Mason->new>.

=head1 PLUGGABLE MASON CLASSES

As of this writing the following Mason classes can be modified with plugins:

    Mason::Compilation
    Mason::Compiler
    Mason::Component
    Mason::Component::ClassMeta
    Mason::Component::InstanceMeta
    Mason::Request
    Mason::Result

=head1 ACKNOWLEDGEMENTS

Thanks to Ricardo SIGNES <rjbs@cpan.org> for L<Dist::Zilla|Dist::Zilla> and
L<Pod::Weaver|Pod::Weaver>, which got me to start thinking in plugins and lent
the plugin and bundle syntax.

