...
The C<< <%init> >> block on lines 13-16 specifies a block of Perl code to run
when this component is first called. In this case it fetches and sorts the list
of articles into a lexical variable C<< @articles >>.

=item %-lines

Lines beginning with a single '%' are treated as Perl rather than HTML. %-lines
are a good way to wrap loops and conditionals around portions of the component.
...
shows two substitution tags. Code within C<< <% >> and C<< %> >> is treated as
a Perl expression, and the result of the expression is output in place.
...
The C<< <%class> >> block on line 1-4 specifies a block of Perl code to place
near the top of the generated component class, outside of any methods. This is
the place to use modules, declare permanent constants/variables, and define
helper methods.

The C<< <%args> >> block on line 7-9 declares a single incoming attribute,
'article'. These lines are equivalent to the Moose syntax
