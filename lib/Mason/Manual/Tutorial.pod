...
This tutorial shows how to build a sample web application using Mason and
L<PSGI|http://plackperl.org/>.
...
As a templating system, the View is Mason's chief responsibility. If you aren't
using Mason for your View, you probably don't need it at all. :)

In contrast, Mason really has nothing to do with the Model except as a
...
The Controller is where things get interesting. Mason and certain plugins
provide some basic Controller facilities, such as parsing web parameters,
specifying how URLs are handled, and forwarding control. However, these
...
However, some people (including the author) find it easier to merge these into
a single layer and have one less set of files to look at.
...
component hierarchy. All components must live under this directory, and any
L<component path|Mason::Manual::Glossary/component_path> is considered relative
to it.
...
We're only going to make a single change to this file, adding Session
middleware:
...
This is a I<component call> - it invokes another component, whose output is
inserted in place.
...
L<internal|internal_component> rather than a top-level component, and cannot be
reached by an external URL. It can only be reached via a component call from
another component.
...
The L<%init|Mason::Syntax/_%init_> block on lines 13-16 specifies a block of
Perl code to run when this component is first called. In this case it fetches
and sorts the list of articles into a lexical variable C<< @articles >>.

=item %-lines

L<%-lines|Mason::Syntax/PERL LINES> - lines beginning with a single '%' - are
treated as Perl rather than HTML.  They are especially good for loops and
conditionals.
...
shows two L<substitution tags|Mason::Syntax/SUBSTITUTION TAGS>. Code within C<<
<% >> and C<< %> >> is treated as a Perl expression, and the result of the
expression is output in place.
...
Next we create C</article/display.mi>:

     1  <%class>
     2  use Date::Format;
     4  my $date_fmt = "%A, %B %d, %Y  %I:%M %p";
     5  </%class>
     6  
     7  <%args>
     8  $.article => (required => 1)
     9  </%args>
    10  
    11  <div class="article">
    12    <h3><% $.article->title %></h3>
    13    <h4><% time2str($date_fmt, $.article->create_time) %></h4>
    14    <% $.article->content %>
    15  </div>

The L<%init|Mason::Syntax/_%class_> block on line 1-4 specifies a block of Perl
code to place near the top of the generated component class, outside of any
methods. This is the place to use modules, declare permanent
constants/variables, and define helper methods.

The L<%args|Mason::Syntax/_%args_> block on line 7-9 declares a single incoming
attribute, 'article'. These lines are equivalent to the Moose syntax

    has 'article' => (required => 1);

and in fact, we could have included this line in the << <%class> >> block; C<<
...
be omitted if the source filtering offends you. :)

=head1 CHAPTER 4

=head2 Templates / content wrapping, autobases, inheritance, method modifiers

Now we create C</new_article.m>, our second top-level page component. It is all
static HTML (for now).
...
rather than a Mason component, and you can skip this chapter.>
...
The C<.pm> extension indicates that this is a I<pure-perl> component. Other
than the 'package' and 'use Moose' lines that are generated by Mason, it looks
just like a regular Perl class. You could accomplish the same thing with a
C<.m> component containing a single C<< <%class> >> block, but this is easier
...
On line 5 we grab the Plack session. C<< $m->req >> is the Plack request
...
form with it - we'll do that in the next chapter. C< $.cmeta >> provides access
to various meta-information about the component, such as its path and the
