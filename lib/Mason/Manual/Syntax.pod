...
a Perl expression in scalar context.
...
A filter list may appear after a << | >> character in a substitution. It must
contain one or more comma-separated names, which are treated as filter methods
on the current component class and applied to the result before it is output.
...
Warning: this syntax cannot be used for conditional or loop statements as
%-lines can be, because Mason will treat the intervening lines as part of the
current method (e.g. 'main').

    <% # WRONG!!! The inner body will end up in the current method. %>
...
with the attribute name, followed optionally by a comma operator (=>) and
either a scalar value (which becomes the default) or a list (which becomes the
Moose attribute properties). e.g. this block
...
absolute as shown above, or relative to the component's path.
...
as << <%args> >>, but the resulting attributes are declared with C<< init_arg
...
Modifies a method with the specified I<name>. See
L<Moose::Manual::MethodModifiers|Moose::Manual::MethodModifiers> for a
description of each modifier.

C<$self> is automatically shifted off for the body of C<< <%after> >>, C<<
<%augment> >>, and C<< <%before> >>. C<$orig> and C<$self> are automatically
shifted off for the body of C<< <%around> >>.
