...
Filters can be used to process the output of a block in a component.
...
component object which returns a filter.
...
Filters can also appear in a limited way inside a regular C<< <% %> >> tag:
    ...
    <% $message_body | NoBlankLines,Trim %>

The filter list appears after a << | >> character and must be one or more
names, separated by comma. The names are treated as methods on the current
component class. With this syntax you cannot use anonymous subroutines or
variables as filters, or pass arguments to filters.
...
Or if you want them available to all components, put them in C<< Base.pm >> at
the top of your component hierarchy, or in your application's Mason::Component
subclass.
...
standard C<< Cache >> filter:

    method Cache ( $key, $set_options ) {
        return Mason::DynamicFilter->new(
            filter => sub {
                my $yield = shift;
                my $cache = $self->cmeta->cache;
