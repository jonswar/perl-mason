...
sub test_component_class_prefix : Test(6) {
    ...
    my $check_prefix = sub {
        ...
        like( $interp->load('/foo'), qr/$regex/, "prefix at beginning of path" );
    };

    $self->add_comp( path => '/foo', component => 'foo' );
    ...
    ok( $interp[0]->component_class_prefix ne $interp[1]->component_class_prefix,
        "different prefixes" );
    ok( $interp[0]->load('/foo') ne $interp[1]->load('/foo'), "different classnames" );
